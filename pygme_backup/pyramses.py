    ...
    def write(self, Suffix=None, dirout=None, mgefile=None, mgemodel=None, verbose=0, mode=None) :
        ...
        if not os.path.isdir(dirout) :
            if mode == "O" :
                ...
                import string
                yesOrNo = raw_input("Do you want it to be created now? (y/N) :")
        ...
        for i in range(self.ngas + self.nsta, self.ngas + self.nsta + self.nhalo) :
            ...
            y = self.pos[i][1]
            z = self.pos[i][2]
            Vx = self.vel[i][0] / 65.57 * 0.9986
            Vy = self.vel[i][1] / 65.57 * 0.9986
            Vz = self.vel[i][2] / 65.57 * 0.9986
        ...
        self.init_mass()
        f_Info = open(dirout + name_Info, 'wb')
